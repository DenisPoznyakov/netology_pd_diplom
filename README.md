...
Реализована возможность входа через GitHub. Для этого используется библиотека `python-social-auth`.  

- Эндпоинты интеграции находятся по пути `/auth/` (включая GitHub login).  
- После успешного входа пользователь перенаправляется на главную страницу или страницу подтверждения.  
...
## Админ-панель и улучшения

- Полный контроль над пользователями, магазинами, категориями, товарами и заказами.
- В ProductAdmin отображаются все связанные магазины через ProductInfo (`get_shops`).
...
- При сохранении запускается Celery-задача generate_thumbnails
- Генерируются миниатюры разных размеров (Pillow)
- Изображения доступны по /media/

## Мониторинг ошибок (Sentry)

- Подключён Sentry SDK для автоматической отправки исключений
- Все необработанные ошибки в Django отправляются в Sentry
- Тестовый эндпоинт: http://127.0.0.1:8000/sentry-debug/ — вызывает ZeroDivisionError и отправляет событие
- DSN: настроен в settings.py
